...
app = Flask(__name__)
app.secret_key = 'clave_secreta_para_un_software_multi_empresa_v3'
PER_PAGE = 5
...
# ... (El resto de las funciones y rutas que no se modifican van aquí) ...
...
def registro():
    if 'user_id' in session: return redirect(url_for('mostrar_inventario'))
    if request.method == 'POST':
        nombre_empresa, username, password = request.form['nombre_empresa'], request.form['username'], request.form['password']
        conn = get_db_connection()
        ...
        cursor = conn.cursor()
        cursor.execute('INSERT INTO empresas (nombre_empresa) VALUES (?)', (nombre_empresa,))
...
def actualizar_stock(producto_id):
    ...
    if not action or action not in ['incrementar', 'decrementar']:
...
def procesar_venta():
    ...
    try:
        for item_id, item_data in cart_data.items():
            ...
            conn.execute('''
                INSERT INTO ventas (transaccion_id, empresa_id, producto_id, usuario_id, cantidad, precio_total, cliente_nombre, cliente_telefono) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def mostrar_factura(transaccion_id):
    conn = get_db_connection()
    items_vendidos = conn.execute('''
        SELECT v.cantidad, v.precio_total, v.cliente_nombre, v.cliente_telefono, p.nombre
        FROM ventas v JOIN productos p ON v.producto_id = p.id
        WHERE v.transaccion_id = ? AND v.empresa_id = ?
    ...
    cliente_nombre = items_vendidos[0]['cliente_nombre']
    cliente_telefono = items_vendidos[0]['cliente_telefono']

    return render_template('factura.html',
                           transaccion_id=transaccion_id,
                           items_vendidos=items_vendidos,
                           empresa=empresa,
                           fecha_venta=primera_venta['fecha_venta'].split(" ")[0],
